...
export const GET = handleRoute(async () => {
  ...
  const interviewStage =
    (byStatus.interview ?? 0) + (byStatus.hr ?? 0) + (byStatus.offer ?? 0)
  ...
  const roadmapProgress = roadmap?.tasks.length
    ? Math.round(
        (roadmap.tasks.filter((t) => t.completed).length /
  ...
  return json({
    ...
    conversionRates: {
      interviewRate:
        applied === 0 ? null : Math.round((interviewStage / applied) * 100),
    ...
    interviews: {
      avgScore:
        interviews.length === 0
          ? null
          : Math.round(
              (interviews.reduce((s, i) => s + (i.score ?? 0), 0) /
                interviews.length) *
                10,
            ) / 10,
      trend: interviews
        .slice(-10)
        .map((i) => ({ score: i.score, date: i.createdAt })),
      count: interviews.length,
    },
    resume: {
      latestAtsScore: resumes.length
