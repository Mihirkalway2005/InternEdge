...
/* Left Pillar 'I' */ /* Top Arm of 'E' with parallel 45° chamfer cut */ /* Middle Arm of 'E' */ /* Bottom Arm of 'E' with parallel 45° chamfer cut */ /* Subtle Brushed Metallic Sheen */ /* Liquid Glass Highlight */
...
export type LogoVariant = "default" | "monochrome-white" | "monochrome-black" | "titanium" | "liquid-glass" | "squircle" | "icon-only"
  ...
  if (variant === "titanium") {
    return (
      ...
      >
        ...
        <svg
          viewBox="0 0 100 100"
          className="w-full h-full relative z-10 filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)]"
        >
          {renderPaths("#FFFFFF")}
        </svg>
      </div>
  ...
  if (variant === "liquid-glass") {
    return (
      ...
      >
        ...
        <svg
          viewBox="0 0 100 100"
          className="w-full h-full relative z-10 filter drop-shadow-[0_2px_8px_rgba(255,255,255,0.3)]"
        >
          {renderPaths("#FFFFFF")}
        </svg>
      </div>
...
export const InternEdgeWordmark: React.FC<InternEdgeWordmarkProps> = ({
  ...
  return (
    <div className={`flex flex-col items-start ${className}`}>
      <span
        className={`font-semibold tracking-[0.35em] ${textColor} uppercase select-none font-sans text-xs md:text-sm`}
      >
        INTERNEDGE
      </span>
      {showTagline && (
      ...
      : size === "md"
        ...
        : size === "lg"
          ...
          : typeof size === "number"
            ? size
            : 32)
  const actualVariant: LogoVariant =
    variant === "icon-only" ? "liquid-glass" : variant
  const shouldShowWordmark = variant === "icon-only" ? false : showWordmark

  return (
    <div className={`inline-flex items-center gap-3.5 ${className}`}>
      <InternEdgeIcon size={calculatedSize} variant={actualVariant} />
      {shouldShowWordmark && <InternEdgeWordmark showTagline={showTagline} />}
    </div>
  )
}
