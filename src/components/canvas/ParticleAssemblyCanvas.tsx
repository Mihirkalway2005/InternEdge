...
// Direct assembly start from left to right
...
export const ParticleAssemblyCanvas: React.FC<ParticleAssemblyCanvasProps> = ({
  ...
  useEffect(() => {
    ...
    const maxX =
      targetPoints.length > 0 ? targetPoints[targetPoints.length - 1].x : width
    ...
    const particles: Particle[] = targetPoints.map((target) => {
      ...
      const delay = normalizedX * 1.8 + Math.random() * 0.2
    ...
    const render = (now: number) => {
      ...
      particles.forEach((p) => {
        if (elapsed < p.delay) {
          ...
          const midX =
            (p.startX + p.targetX) / 2 +
            Math.cos(p.curvedAngle) * p.curvedDistance
          const midY =
            (p.startY + p.targetY) / 2 +
            Math.sin(p.curvedAngle) * p.curvedDistance

          const invT = 1 - easeProgress
          p.x =
            invT * invT * p.startX +
            2 * invT * easeProgress * midX +
            easeProgress * easeProgress * p.targetX
          p.y =
            invT * invT * p.startY +
            2 * invT * easeProgress * midY +
            easeProgress * easeProgress * p.targetY
