"use client" /* Background Hero Dashboard Preview (Materializes slowly through depth and blur) */ /* Unified Particle-to-Solid Canvas (Renders assembly & fuses into solid crisp title) */ /* Subtitle & Keynote CTA (Appears smoothly after solid text fusion completes) */ /* Keynote Begin Affordance Arrow */
...
export const Chapter01Opening: React.FC<Chapter01Props> = ({
  onProceed,
  onAudioMorph,
}) => {
  ...
  return (
    <section className="relative w-full h-screen bg-[#050505] flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div
        ...
        animate={
          isAssembled
            ? { opacity: 0.18, scale: 1, filter: "blur(10px)" }
            : { opacity: 0 }
        }
